function [anterdir,posterdir,dorsaldir,ventraldir,leftdir,rightdir]=apdvlr_axes_2(coords,guess,scale_factor,str, normal_vector,chosen_point)
...
chosen_point(3) = midpoint(3);
dirfactor = sign(dot(chosen_point'-midpoint,coefs(:,1)));

anterdir=1.*coefs(:,1).*dirfactor;
posterdir=-1.*coefs(:,1).*dirfactor;
