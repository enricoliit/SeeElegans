...
for i=1:size(added,1)
    ...
    for j=1:numel(neurons)
        ...
        try
            velMatrix3(i,j,1)=added(i,2)-neurons(j).coords(time_id,1);
            velMatrix3(i,j,2)=added(i,3)-neurons(j).coords(time_id,2);
            velMatrix3(i,j,3)=added(i,4)-neurons(j).coords(time_id,3);
        catch
            velMatrix3(i,j,1:3)=NaN;
        end
    end
        ...
        for j=1:numel(neurons)
            try
                if ~isnan(neurons(j).coords(t_id,1))  && ~isnan(distMatrix3(i,j,1))
                    ...
                    single_weight = 1/sum(distMatrix3(i,j,:).^2);
                    if ~isnan(addition)
%                         reconstruction{i}(t_id,1:3) = reconstruction{i}(t_id,1:3) + addition;
                        reconstruction{i}(t_id,1:3) = reconstruction{i}(t_id,1:3) + addition .* single_weight;
...
%         reconstruction{i}(t_id,1:3)=reconstruction{i}(t_id,1:3)./total;
        reconstruction{i}(t_id,1:3)=reconstruction{i}(t_id,1:3)./total_weights;
    end
end
...
search_factor = 0.4;                   % with respect to automati search
    ...
    if 1
        ...
        for i=1:size(reconstruction{add_id},1)
            ...
            if ~isnan(row)
                ...
                filtered_image=imgaussfilt(wholestack(row1:row2,col1:col2,plane,i),1); % SCALA
                ...
                ROI_m = imgaussfilt((median(wholestack(row_max1:row_max2,col_max1:col_max2,plane,i:i+3),4)-background)./1,1);
                %%-------------------------------------------------------------------------
                ROIs{iRoi}=[ROI_m];
                iRoi=iRoi+1;
                %%-------------------------------------------------------------------------
                ...
                try
                    ROI_u = imgaussfilt((median(wholestack(row_max1:row_max2,col_max1:col_max2,plane+1,i:i+3),4)-background)./1,1);
                catch
                    ROI_u = NaN.*ones(size(ROI_m));
                end
                try
                    ROI_d = imgaussfilt((median(wholestack(row_max1:row_max2,col_max1:col_max2,plane-1,i:i+3),4)-background)./1,1);
                catch
