function mutual_position_based_links=get_mpbl(distmatrix_s,coords_t,tmax_local,xy_pixel_size,z_pixel_size,max_gap_closing_mutual_distance)

distmatrix_s_mean=squeeze(mean(distmatrix_s,3,'omitnan'));
taken=[];
h = waitbar(0, 'Merging tracks based on mutual distance ...');
for i=1:size(coords_t,3)
    waitbar(i./size(coords_t,3), h);
    tmax_i=coords_t(:,4,i);
    tmax_i=tmax_i(~isnan(tmax_i));
    tmax_i=tmax_i(end);
    if tmax_i<tmax_local
        link=0;
        ...
        for j=loop_set
            tmax_j=coords_t(:,4,j);
            tmax_j=tmax_j(~isnan(tmax_j));
            tmax_j=tmax_j(end);
            if tmax_j>=tmax_i
                current_difference=sqrt( (xy_pixel_size.*(distmatrix_s_mean(i,:,1)-distmatrix_s_mean(j,:,1))).^2 + (xy_pixel_size.*(distmatrix_s_mean(i,:,2)-distmatrix_s_mean(j,:,2))).^2 + (z_pixel_size.*(distmatrix_s_mean(i,:,3)-distmatrix_s_mean(j,:,3))).^2);
